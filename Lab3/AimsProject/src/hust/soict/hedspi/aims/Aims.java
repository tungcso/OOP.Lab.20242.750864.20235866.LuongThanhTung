	        ...
	        for (Media media : store.getItemsInStore()) {
	            if (media.getTitle().equalsIgnoreCase(title)) {
	                ...
	                if (media instanceof DigitalVideoDisc) {
	                    System.out.println("Number of DVDs in cart: " + cart.getItemsOrdered().stream().filter(item -> item instanceof DigitalVideoDisc).count());
	                }
	                break;
	            }
	        }
	    ...
	    public static void addMediaToStore() {
	        ...
	        switch (type) {
	            ...
	            case "cd":
	                ...
	                CompactDisc cd = new CompactDisc(title, category, cost, 0, "IDK", artist); // Length and director might be unknown initially
	                store.addMedia(cd);
